...
// Adapted from React Bits MagicBento for Next.js
// Changes: added "use client" directive; added `cards` prop to BentoProps
      ...
      <BentoCardGrid gridRef={gridRef}>
        <div className="card-responsive grid gap-2">
          {cardData.map((card, index) => {
            const baseClassName = `card flex flex-col justify-between relative min-h-[160px] sm:min-h-[200px] w-full max-w-full p-5 rounded-[20px] border border-solid font-light overflow-hidden transition-colors duration-300 ease-in-out hover:-translate-y-0.5 hover:shadow-[0_8px_25px_rgba(0,0,0,0.15)] ${enableBorderGlow ? 'card--border-glow' : ''}`
            ...
            const cardContent = (
              <>
                <div className="card__header flex justify-between gap-3 relative text-white">
                  <span className="card__label font-mono text-[11px] tracking-widest uppercase opacity-40">{card.label}</span>
                </div>
                <div className="card__content flex flex-col relative text-white mt-auto">
                  <h3 className={`card__title font-medium text-[15px] leading-snug m-0 mb-1.5 ${textAutoHide ? 'text-clamp-1' : ''}`}>
                    {card.title}
                  </h3>
                  <p className={`card__description text-[13px] leading-[1.55] opacity-50 ${textAutoHide ? 'text-clamp-2' : ''}`}>
                    {card.description}
                  </p>
                </div>
              </>
